                     ...
                     _In_ int       nCmdShow)
{
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_HOMEWORK));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_HOMEWORK);
...
void DrawBK(HDC hdc, int x1, int y1, int x2, int y2)
        {
            ...
            char m[100] = "关闭中文输入法，选择菜单[文件/开始游戏]  版权所有：YJM 2021";
            TextOut(hdc, x1 -75, y2 + 15, m, lstrlen(m));
    }
...
void ShowScoring(HDC hdc, int x, int y, int iScoring, int iFail, int iTime)
{
    ...
    void GameOver(HDC hdc, int x, int y);
    {
        ...
        OldColor = SetTextColor(hdc, NewColor);        //设置文本颜色为红色
    ...
    void Fire(HDC hdc, int x, int y1, int y2);
    {
        ...
        hOldPen = (HPEN)SelectObject(hdc, hNewPen);                     //选择新画笔、保留旧画笔
        ...
        SelectObject(hdc, hNewPen2);                          //选择新画笔、保留旧画笔
        MoveToEx(hdc, x, y1, NULL);
        LineTo(hdc, x, y2);
    ...
    case WM_PAINT:
        {
            ...
            HBRUSH hOldBrush, hNewBrush = CreateSolidBrush(RGB(240, 255, 255));
