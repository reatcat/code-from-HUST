...
status Juge_Hole(int arr[][9], int n, int m, int k, headnode H, bignode F) {
  for (int i = 1; i <= 9; i++)
    if (i != k &&
        SuDoKu_Judge(arr, n, m, i) == OK)  //判断填入的数是否符合数独规则
    {
      arr[n][m] = i;
...
status Dig_Hole(int arr[][9], int n, int m, int k, headnode H, bignode F) {
  ...
  if (counnt == 0)
    ...
    if (Juge_Hole(arr, n - 1, m - 1, k, H, F) ==
        FALSE)  //判断传入的是否有其他解
    {           //没解复原
      arr[n - 1][m - 1] = k;
      hole[n - 1][m - 1] = 1;
      ...
      if (next_hole == 0)
        return OK;
      if (Dig_Hole(arr, next_hole / 100, (next_hole / 10) % 10, next_hole % 10,
                   H, F) == OK)
        return OK;
      else {
        while (sig--) {
          ...
          if (next_hole == 0)
            return OK;
          if (Dig_Hole(arr, next_hole / 100, (next_hole / 10) % 10,
                       next_hole % 10, H, F) == OK)
            return OK;
          else
            return FALSE;
