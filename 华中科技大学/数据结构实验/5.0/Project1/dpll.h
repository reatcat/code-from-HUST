  ...
  if (flag > 0) {  //传进来为正
    ...
    while (p) {
      ...
      if (p->fu_mark == 1 && p->outside_fu->mark == 1 &&
          p->inside_fu->mark == 1 && p->outside_fu->count != 0) {
        p->inside_fu->mark = 0;
        p->outside_fu->count--;
      }
      if (p->zheng_mark == 1 && p->outside_zheng->mark == 1) {  //为正，删除子句
    ...
    while (p) {
      ...
      if (p->zheng_mark == 1 && p->outside_zheng->mark == 1 &&
          p->inside_zheng->mark == 1 && p->outside_zheng->count != 0) {
        p->inside_zheng->mark = 0;
        p->outside_zheng->count--;
      }
      if (p->fu_mark == 1 && p->outside_fu->mark == 1) {  //为负，删除子句
        p->outside_fu->mark = 0;
  ...
  if (flag > 0) {
    ...
    while (p) {
      ...
      if (p->fu_mark == 1 && p->outside_fu->mark == 1 &&
        ...
        p->inside_fu->mark = 1;
        p->outside_fu->count++;
      } else if (p->zheng_mark == 1 && p->outside_zheng->mark == 0 &&
                 p->outside_zheng->flag == flag) {  //为正，恢复子句
    ...
    while (p) {
      ...
      if (p->zheng_mark == 1 && p->outside_zheng->mark == 1 &&
        ...
        p->outside_zheng->count++;
      } else if (p->fu_mark == 1 && p->outside_fu->mark == 0 &&
                 p->outside_fu->flag == flag) {  //为负，恢复子句
        p->outside_fu->mark = 1;
        p->outside_fu->flag = 0;
      }
      p = p->next;
  ...
  if (JudgeGroup(Head) == OK)
    ...
    {  //出现空子句就恢复空子句并回溯
      ...
      if (Dpll_youhua(Head, next_count, G) == OK)
        ...
        if (Dpll_youhua(Head, -next_count, G) == OK) {
          if (next_count > 0)
            mark_ture[next_count] = -1;
          else
            mark_ture[-next_count] = 1;
          return OK;
        } else {  //负数也测试失败，则标记该假设，返回上一个假设
          Recovery(G, -next_count);
